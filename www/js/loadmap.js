...
function initialize() {
	...
	google.maps.event.addListener(kmlLayer, 'click', function(kmlEvent) {
		var text = '<div class="well well-sm" id="garage-info">'
                ...
                +'<div class="input-group" id="slider-div">'
				+'<span class="input-group-btn"><button onclick="decrease();" class="btn btn-primary" type="button">-</button></span>'
				...
				+'<span class="input-group-btn"><button onclick="increase();" class="btn btn-primary" type="button">+</button></span>'
                +'</div>'
                +'<div class="row">'
                +'<div class="col-6 col-sm-6 col-lg-6"><input type="number" class="form-control" id="slider-value" value="2"></div><span class="help-block">hour(s)</span>'
                +'</div>'
...
function decrease(){
	var currentVal = $('#slider-value').val();
	if (currentVal > 0){
		$('#slider-value').val(parseInt(currentVal) - 1).change();
	}
}
function increase(){
	var currentVal = $('#slider-value').val();
